...
const ResourceHarvests: React.FC<ResourceHarvestsProps> = ({ harvestAssets, currentPage, totalPages, setCurrentPage, itemsPerPage }) => (
  <div className="mx-auto border-b border-l border-theme-pan-navy bg-theme-pan-champagne rounded-bl rounded-tr py-4 mt-6">
    <h1 className="text-lg pl-4 font-bold text-left">Resource Harvests</h1>
    <div className="mx-8 rounded-sm pb-3 pt-2">
      <table className="min-w-full divide-y divide-theme-pan-navy">
        <thead className="bg-theme-pan-champagne">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-theme-pan-navy uppercase tracking-wider">Crypto Resources</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-theme-pan-navy">
          {harvestAssets.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((entry, index) => (
            <tr key={index}>
              <td className="px-6 py-2 text-sm whitespace-nowrap">{entry.date}</td>
              <td className="px-6 py-2 text-sm whitespace-nowrap">{entry.symbols}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div className="flex justify-between mt-4">
        <button
          ...
          className="px-4 py-2 bg-theme-pan-champagne text-sm text-theme-pan-navy rounded disabled:opacity-50"
        >
          Previous
        </button>
        <span className="text-theme-pan-navy text-sm">{`Page ${currentPage} of ${totalPages}`}</span>
        <button
